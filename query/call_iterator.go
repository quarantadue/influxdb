...
func newMeandBIterator(input Iterator, opt IteratorOptions) (Iterator, error) {
	...
	case UnsignedIterator:
		createFn := func() (UnsignedPointAggregator, FloatPointEmitter) {
			fn := NewUnsignedSliceFuncFloatReducer(UnsignedMeandBReduceSlice)
			return fn, fn
		}
		return newUnsignedReduceFloatIterator(input, opt, createFn), nil
	default:
...
// UnsignedMeandBReduceSlice returns the meandb value within a window.
func UnsignedMeandBReduceSlice(a []UnsignedPoint) []FloatPoint {
